...
{
    const PERMALINK_POST_USE_CATEGORIES = 'mageos_blog/permalink/post_use_categories';
    ...
    public function resolve($path)
    {
        ...
        unset($pathInfo[0]);
        $pathInfo = array_values($pathInfo); // Reindex array after unset

        if (!count($pathInfo)) {
            return ['id' => 1, 'type' => Url::CONTROLLER_INDEX];
        }

        // Handle /blog/page/X for index pagination
        if (isset($pathInfo[0]) && $pathInfo[0] === 'page' && !empty($pathInfo[1]) && is_numeric($pathInfo[1])) {
            return [
                'id' => 1, // Assuming this is the index page
                'type' => Url::CONTROLLER_INDEX,
                'params' => ['page' => (int)$pathInfo[1]] // Pass page as a parameter
            ];
        }

        // Handle /blog/category/[category]/page/X for category pagination
        if (isset($pathInfo[0]) && $pathInfo[0] === 'category'
            && !empty($pathInfo[1])
            && ($categoryId = $this->_getCategoryId($pathInfo[1]))
        ) {
            $result = ['id' => $categoryId, 'type' => Url::CONTROLLER_CATEGORY];

            // Check for /page/X suffix
            if (isset($pathInfo[2]) && $pathInfo[2] === 'page'
                && !empty($pathInfo[3]) && is_numeric($pathInfo[3])
            ) {
                $result['params'] = ['page' => (int)$pathInfo[3]];
            }
        ...
        if ($pathInfo[0] == $this->url->getRoute(Url::CONTROLLER_SEARCH)) {
            return ['id' => empty($pathInfo[1]) ? '' : $pathInfo[1], 'type' => Url::CONTROLLER_SEARCH];
        } elseif ($pathInfo[0] == $this->url->getRoute(Url::CONTROLLER_TAG)
            && !empty($pathInfo[1])
            && $tagId = $this->_getTagId($pathInfo[1])
        ) {
            return ['id' => $tagId, 'type' => Url::CONTROLLER_TAG];
        } else {
            $controllerName = null;
            if (Url::PERMALINK_TYPE_DEFAULT == $this->url->getPermalinkType()) {
                $controllerName = $this->url->getControllerName($pathInfo[0]);
                unset($pathInfo[0]);
            }
    ...
    {
        $storeId = (int)$this->storeId ?: (int)$this->storeManager->getStore()->getId();
