...
class Wordpress extends AbstractImport
{
        ...
        foreach ($result as $data) {
            ...
            foreach ($result2 as $data2) {
                if ($data2['featured_img']) {
                    $data['featured_img'] = \MageOS\Blog\Model\Post::BASE_MEDIA_PATH . '/' . $data2['featured_img'];
                    break;
                }
            }
                    ...
                    if ($data2['featured_img']) {
                        ...
                        if (is_array($tmpArr)) {
                            foreach ($tmpArr as $item) {
                                ...
                                if ($item) {
                                    $data['featured_img'] = \MageOS\Blog\Model\Post::BASE_MEDIA_PATH . '/' . $item;
                                    break 2;
                                }
            ...
            */
            ...
            $creationTime = (string)$data['post_date'];
            ...
            $content = preg_replace(
                ...
                'src="$4{{media url="mageos_blog$6$8"}}$9"',
                $content
            );
            ...
            $data = [
                ...
                'content_heading' => '',
                'content' => $this->cleanDiviContent($content),
            ...
            try {
                ...
                foreach ($resultComments as $comments) {
                    ...
                    $commentData = [
                        ...
                        'status' => \MageOS\Blog\Model\Config\Source\CommentStatus::APPROVED,
                        'author_type' => \MageOS\Blog\Model\Config\Source\AuthorType::GUEST,
    ...
    /**
     * Clean Divi builder shortcodes and empty tags from content,
     * then wrap the cleaned content in a div with class "wp-imported"
     *
     * @param string $content The content to clean
     * @return string Cleaned content wrapped in div
     */
    function cleanDiviContent($content) {
        // Remove Divi shortcodes - matches patterns like [et_pb_section...][/et_pb_section]
        $content = preg_replace('/\[\/?et_pb_[^\]]*\]/', '', $content);

        // Remove HTML comments including wp:divi/placeholder comments
        $content = preg_replace('/<!--[\s\S]*?-->/', '', $content);

        // Remove empty paragraphs and other common empty tags
        ...
        // Wrap the cleaned content in a div with class "wp-imported"
        $content = '<div class="wp-imported">' . $content . '</div>';
