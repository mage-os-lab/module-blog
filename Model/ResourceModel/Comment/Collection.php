...
namespace MageOS\Blog\Model\ResourceModel\Comment;
...
class Collection extends AbstractCollection
{
    ...
    {
        $this->_init(\MageOS\Blog\Model\Comment::class, \MageOS\Blog\Model\ResourceModel\Comment::class);
    ...
    {
        return $this
            ->addFieldToFilter('status', \MageOS\Blog\Model\Config\Source\CommentStatus::APPROVED);
    }
     ...
     * @param array|int|string|\MageOS\Blog\Model\Post  $post
     * @return $this
     */
    public function addPostFilter($post)
    {
        if (!$this->getFlag('post_filter_added')) {
            if ($post instanceof \MageOS\Blog\Model\Post) {
                $post = [$post->getId()];
            }
            ...
            $tableName = $this->getTable('blog_post');
    ...
    protected function _renderFiltersBefore()
    {
        if ($this->getFilter('store') && !$this->getFlag('store_filtered')) {
            $this->getSelect()->join(
                ['store_table' => $this->getTable('blog_post_store')],
