...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
     ...
     * @inheritDoc
     */
    protected $_eventPrefix = 'mageos_blog_post_collection';
    ...
    /**
     * @var \MageOS\Blog\Api\CategoryRepositoryInterface|null
     */
     ...
     * @param \MageOS\Blog\Api\CategoryRepositoryInterface|null $categoryRepository
     */
    public function __construct(
        ...
        \MageOS\Blog\Api\CategoryRepositoryInterface $categoryRepository = null
    ) {
        ...
        $this->categoryRepository = $categoryRepository ?: $objectManager->create(
            \MageOS\Blog\Api\CategoryRepositoryInterface::class
        );
    }
    ...
    {
        parent::_construct();
        $this->_init(\MageOS\Blog\Model\Post::class, \MageOS\Blog\Model\ResourceModel\Post::class);
     ...
     * @param array|int|\MageOS\Blog\Model\Category  $category
     * @return $this
     */
    public function addCategoryFilter($category)
    {
        if (!$this->getFlag('category_filter_added')) {
            if ($category instanceof \MageOS\Blog\Model\Category) {
            ...
            $tableName = $this->getTable('blog_category');
    ...
    public function addSearchFilter($term)
    {
        ...
        $select = $connection->select()
            ->from(
                ['p' => $this->getTable('blog_post')],
                ['post_id']
            )->joinInner(
                ['pt' => $this->getTable('blog_post_tag')],
                'p.post_id = pt.post_id',
                ['']
            )->joinInner(
                ['t' => $this->getTable('blog_tag')],
                't.tag_id = pt.tag_id',
                ['tag_title' => 'title']
     ...
     * @param array|int|string|\MageOS\Blog\Model\Tag  $tag
     * @return $this
     */
    public function addTagFilter($tag)
    {
        if (!$this->getFlag('tag_filter_added')) {
            if ($tag instanceof \MageOS\Blog\Model\Tag) {
                $tag = [$tag->getId()];
            }
            ...
            $connection = $this->getConnection();
            $tableName = $this->getTable('blog_tag');
    ...
    protected function _afterLoad()
    {
        ...
        if (count($items)) {
            $connection = $this->getConnection();
            $tableName = $this->getTable('blog_post_store');
            $select = $connection->select()
                ->from(['cps' => $tableName])
            ...
            foreach ($map as $key => $property) {
                $tableName = $this->getTable('blog_post_' . $key);
                $select = $connection->select()
                    ->from(['cps' => $tableName])
                    ->where('cps.post_id IN (?)', $items);
        ...
        foreach (['store', 'category', 'tag', 'author', 'relatedproduct'] as $key) {
            if ($this->getFilter($key)) {
                ...
                $this->_eventManager->dispatch(
                    'mageos_blog_post_collection_render_filter_join',
                    ['join_options' => $joinOptions]
                );
                $this->getSelect()->join(
                    [$key . '_table' => $this->getTable('blog_post_' . $key)],
