...
abstract class Actions extends \Magento\Backend\App\Action
{
    ...
    public function _saveAction()
    {
        ...
        try {
            ...
            $this->_eventManager->dispatch('roweb_' . $this->getRequest()->getModuleName() . '_' . $this->getRequest()->getControllerName()  . '_form_before_save', ['model' => $model]);
            ...
            $this->_eventManager->dispatch('roweb_' . $this->getRequest()->getModuleName() . '_' . $this->getRequest()->getControllerName()  . '_form_after_save', ['model' => $model]);
    ...
    protected function _deleteAction()
    {
        ...
        try {
            foreach ($ids as $id) {
                ...
                //$this->_objectManager->create($this->_modelClass.'Repository')->deleteById($id);
            }
        } catch (\Magento\Framework\Exception\LocalizedException $e) {
            $error = true;
    ...
    protected function _getModel($load = true)
    {
        if (is_null($this->_model)) {
            ...
            if ($id && $load) {
                $this->_model->load($id);
                $this->_eventManager->dispatch('roweb_' . $this->getRequest()->getModuleName() . '_' . $this->getRequest()->getControllerName()  . '_form_load_model_after', ['model' => $this->_model]);
            }
        }
        return $this->_model;
    }
     ...
     * @return mixed
     */
    protected function getFilterInput($filterRules, $validatorRules, $data): mixed
    {
     ...
     * @param array $fields
     */
    protected function prepareImagesBeforeSave($model, array $fields): void
    {
        $data = $model->getData();
                ...
                if (!empty($data[$key]['delete'])) {
                    ...
                    if (isset($data[$key][0]['name']) && isset($data[$key][0]['tmp_name'])) {
                        ...
                        if (isset($data[$key][0]['url']) && false !== strpos($data[$key][0]['url'], '/media/')) {
                             ...
                             *    /media/.renditions/mageos_blog/a.png
                             *    http://domain.com/media/mageos_blog/tmp/a.png
                             */
