...
class Run extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        try {
            if (empty($type)) {
                throw new \Exception('Blog import type is not specified.', 1);
            }
            ...
            $import = $this->_objectManager->create('\MageOS\Blog\Model\Import\\'.$_type);
            ...
            if ($stats->getData('imported_count')) {
                ...
                if (!$stats->getData('skipped_count')) {
                    ...
                    throw new \Exception('Can not make import.', 1);
                }
            }
